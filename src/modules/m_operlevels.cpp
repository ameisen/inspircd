	...
	public:
		...
		ModResult OnKill(User* source, User* dest, const std::string &reason) override
		{
			...
			if (dest->IsOper() && source->IsOper())
			{
				...
				{
					if (source->as<LocalUser>()) ServerInstance->SNO->WriteGlobalSno('a', "Oper %s (level %ld) attempted to /kill a higher oper: %s (level %ld): Reason: %s",source->nick.c_str(),source_level,dest->nick.c_str(),dest_level,reason.c_str());
